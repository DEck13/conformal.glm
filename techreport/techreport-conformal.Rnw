...
## Gamma region
dat.gamma <- cbind(dat, conformal.gamma)
upr <- dat.gamma$upr
lwr <- dat.gamma$lwr
x <- dat$age
y <- dat$glyhb
...
title3d(main="Gamma conformal", cex=2, line = 2)
...
for(j in 1:length(dat_by_age)) {
  ...
  # polygon is in (y,z)
  polygon3d(x=xvals, y=yvals, z=zvals, coords=c(2,3), fill=TRUE, col="red", random=FALSE)
}
