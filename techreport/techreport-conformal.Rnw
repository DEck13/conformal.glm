...
gamma_simulator <- function(beta, n = 500, alpha = 0.10, bins = 3, 
  ...
  y <- rep(0,n)
  dat <- data.frame(Date=as.Date(character()),
    File=character(), User=character(), stringsAsFactors=FALSE) 
...
system.time(conformal.gamma <- 
  conformal.glm(m.gamma.log, cores = 6, bins = 1, 
    method = "both"))
conformal.gamma <- conformal.gamma$paraconfbin
...
system.time(conformal.gaussian <- 
  conformal.glm(m.gaussian, cores = 6, bins = 1, 
    method = "both"))
conformal.gaussian <- conformal.gaussian$paraconfbin
