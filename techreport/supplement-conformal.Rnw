...
lines(x[ix], paraCI[ix, 1], type = "l", col = "red", lwd = 1.5)
lines(x[ix], paraCI[ix, 2], type = "l", col = "red", lwd = 1.5)
axis(2)
...
points(x, y, pch = 19, col = rgb(0,0,0,alpha=0.2))
lines(x[ix], LSLW[ix, 1], type = "l", col = "red", lwd = 1.5)
lines(x[ix], LSLW[ix, 2], type = "l", col = "red", lwd = 1.5)
axis(1); axis(2)
...
plot.window(xlim = c(0,1), ylim = c(0,max(y)))
points(x, y, pch = 19, col = rgb(0,0,0,alpha=0.2))
lines(x[ix], HDCI[ix, 1], type = "l", col = "red", lwd = 1.5)
lines(x[ix], HDCI[ix, 2], type = "l", col = "red", lwd = 1.5)
axis(1)
...
nonpara.local.coverage.inx <- local.gamma.500.3.2[36:60, 1]
LS.local.coverage.inx <- local.gamma.500.3.2[67:91, 1]
LSLW.local.coverage.inx <- local.gamma.500.3.2[98:122, 1]
HD.local.coverage.inx <- local.gamma.500.3.2[129:153, 1]
...
legend(0.20, 0.75, legend=c("parametric", 
...
Figures~\ref{plot.regions.150.2.0.5}-\ref{plot.regions.150.2.2} in 
Section~\ref{sec:gammaplots}.  These figures correspond to data generated 
...
mtext("Simulation Setting A", side = 3, cex = 1.5, line = 2, outer = TRUE)
@
\end{center}
...
lines(inx, para.inx.gamma.150[1:25], col = "red", lty = 1, lwd = 1.5)
lines(inx, nonpara.inx.gamma.150[1:25], col = "green", lty = 2, lwd = 1.5)
lines(inx, LSLW.inx.gamma.150[1:25], col = "purple", lty = 1, lwd = 1.5)
lines(inx, LS.inx.gamma.150[1:25], col = "black", lty = 2, lwd = 1.5)
lines(inx, HD.inx.gamma.150[1:25], col = "blue", lty = 4, lwd = 1.5)
...
plot.new()
plot.window(xlim = c(0, 1), ylim = c(0.25, 1.00))
lines(inx, para.inx.gamma.150[76:100], col = "red", lty = 1, lwd = 1.5)
lines(inx, nonpara.inx.gamma.150[76:100], col = "green", lty = 2, lwd = 1.5)
lines(inx, LSLW.inx.gamma.150[76:100], col = "purple", lty = 1, lwd = 1.5)
lines(inx, LS.inx.gamma.150[76:100], col = "black", lty = 2, lwd = 1.5)
lines(inx, HD.inx.gamma.150[76:100], col = "blue", lty = 4, lwd = 1.5)
abline(h = 0.90, lty = 1, col = "grey", lwd = 1.5)
axis(2)
...
legend(1.75, 0.45, legend=c("misspecified parametric", 
...
\caption{This figure compares the performance of the 
  ...
  least squares, and 
  least squares locally weighted conformal prediciton region and the 
  misspecified highest density prediction region when $n = 150$ and the 
  number of bins equals 2.  
  The specific diagnostics used to compare these prediciton regions is the 
    area (top-left panel),
    prediction error (top-right panel), and
    the coverage probability with respect to binning (bottom row) 
...
The following function makes the conformal prediction region plots for our 
Gamma simulations under correct parametric conformal model specification.
...
makeplots  <- function(n = 500, bins = 3, shape = 1, beta){
  ...
  p2.tibs <- conformal.pred(x = cbind(x,x^2,x^3), y = y, 
    ...
    train.fun = train.fun, predict.fun = predict.fun, 
    mad.train.fun = mad.train.fun,
    mad.predict.fun = mad.predict.fun, alpha = alpha)
  LSLWCI <- cbind(p2.tibs$lo, p2.tibs$up)  
...
\begin{figure}[h!]
\begin{center}
<<plot-regions-150-2-50, fig.height = 2,  cache = TRUE>>=
makeplots(n = 150, shape = 50, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: Data is generated as Gamma with $n = 150$, 
  shape = $50$, and bins = $2$. 
  The regions are depicted as follows: 
    ...
    nonparametric conformal prediction region (middle-right panel),
    LS conformal prediction region (far-right panel).}
\label{plot.regions.150.2.50}
\end{figure}

\begin{figure}[h!]
\begin{center}
<<plot-regions-150-2-100, fig.height = 2,  cache = TRUE>>=
makeplots(n = 150, shape = 100, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: Data is generated as Gamma with $n = 150$, 
  shape = $100$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (far-left panel),
    LSLW conformal prediction region (middle-left panel),
    nonparametric conformal prediction region (middle-right panel),
    LS conformal prediction region (far-right panel).}
\label{plot.regions.150.2.100}
\end{figure}
...
The following function makes the conformal prediction region plots for our 
Gamma simulations under incorrect parametric conformal model specification.
...
The following function makes the conformal prediction region plots for our 
linear regression simulations under correct parametric, LS, and LSLW conformal 
model specification.
...
\newpage
\section{Depiction of conformal prediction regions that appears in \citet{eck2019conformal}}
...
lines(x[ix], paraCI[ix, 2], col = "red", lwd = 1.5)
axis(2)
mtext("parametric", side = 3, cex = 1, line = 1)
...
lines(x[ix], LSLWCI[ix, 1], col = "red", lwd = 1.5)
lines(x[ix], LSLWCI[ix, 2], col = "red", lwd = 1.5)
mtext("LSLW", side = 3, cex = 1, line = 1)

## nonparametric conformal
plot.nonparametric(region = nonparaCI, x = x, 
  y = y, bins = bins)
mtext("nonparametric", side = 3, cex = 1, line = 1)
