...
prediction regions even when the model is misspecified.  
\end{abstract}
...
This manuscript provides all of the code to reproduce the analyses which 
appear in the paper \citet{eck2019conformal} and the README file in the 
R package \citet{eck2018conformalR}.  Several additional analyses are 
provided.  In these analyses we investigate the performance of prediction 
regions under model misspecification.  The underlying 
data is generated via a Gamma distribution and parametric prediction regions 
are obtained using a cubic regression model assuming homoscadistic normal 
errors.  The cubic fit is chosen because it is intuitive and it fits the Gamma 
data better than a simple linear regression model.  

Our goal in this manuscript is to demonstrate the advantages of our parametric 
conformal prediction region \citep{eck2019conformal} compared to the 
nonparametric conformal prediction region \citep{lei2014distribution}, the 
least squares (LS) conformal prediction region \citep{lei2014distribution} 
obtained from conformalized residual scores, the least squares locally 
weighted (LSLW) conformal prediction region 
\citep[Section 5.2]{lei2014distribution} obtained from conformalized locally 
...
In our simulations we find that the parametric conformal prediction 
...
maintains finite-sample local validity.  It therefore achieves finite-sample 
marginal validity.  These are the noted benefits of these conformal prediction 
regions \citep{lei2014distribution, eck2019conformal}.  We find that the LS 
conformal prediction region obtains marginal validity but performs poorly 
when the model is misspecified. 
%None of these prediction regions exhibit 
%finite-sample conditional validity (nominal coverage at each predictor value).  
%That being said, the parametric conformal prediction region is the closest to 
%finite-sample conditionally valid in these simulations.  

%The parametric conformal prediction region exhibits the lowest prediction 
%error (defined in the Diagnostics subsection) among the candidate regions 
%and is often one of the samllest in size.  Our analyses provide favorable 
%vidence that parametric conformal prediction regions provide desirable 
%statistical properties relative to competitors, even in the presence of 
%modest model misspecification.

In the next Section, we reproduce the analysis in the README file 
corresponding to the R package \citet{eck2018conformalR}.  In this analysis 
the HD prediction region is an oracle region of minimal length 
(up to randomness).  The parametric conformal prediction region is only 
slightly smaller in size.  On the other hand, the parametric conformal 
prediction gives finite-sample locally (albeit a bit conservative) coverage 
while the HD prediction region does not do so.
...
# axis labels
mtext("horizontal axis", side = 1, line = 2.5, outer = TRUE, cex = 2)
...
axis(1); axis(2)
mtext("horizontal axis", side = 1, cex = 1.25, line = 2, outer = TRUE)
...
makeplots  <- function(n = 500, bins = 3, shape = 1, beta){
  ...
  ## obtain HD prediction region (functionality for 
  ## different families and link functions)
  betaMLE <- coefficients(fit)
  shapeMLE <- as.numeric(gamma.shape(fit)[1])
  rateMLE <- cbind(1, newdata) %*% betaMLE * shapeMLE
  HDCI <- do.call(rbind, lapply(1:nrow(newdata), function(j){ 
  ...
  ## create plots
  ix <- sort(x, index.return = TRUE)$ix
  par(mfrow = c(3,2), oma = c(4,4,0,0), mar = c(1,1,1,1))

  ## parametric conformal
  plot.new()
  plot.window(xlim = c(0, 1), ylim = c(-2, max(y)))
  points(x, y, pch = 19, col = "gray")
  lines(x[ix], paraCI[ix, 1], col = "red")
  lines(x[ix], paraCI[ix, 2], col = "red")
  axis(2)
  ...
  plot.window(xlim = c(0,1), ylim = c(-2, max(y)))
  points(x, y, pch = 19, col = "gray")
  lines(x[ix], HDCI[ix, 1], col = "red")
  lines(x[ix], HDCI[ix, 2], col = "red")

  ## LSLW conformal
  plot.new()
  plot.window(xlim = c(0, 1), ylim = c(-2, max(y)))
  points(x, y, pch = 19, col = "gray")
  lines(x[ix], LSLWCI[ix, 1], col = "red")
  lines(x[ix], LSLWCI[ix, 2], col = "red")
  axis(2)
  ...
  plot.window(xlim = c(0, 1), ylim = c(-2, max(y)))
  points(x, y, pch = 19, col = "gray")
  lines(x[ix], LSCI[ix, 1], col = "red")
  lines(x[ix], LSCI[ix, 2], col = "red")
  axis(1); axis(2)

  mtext("horizontal axis", side = 1, cex = 1.25, line = 2, outer = TRUE)
  mtext("y", side = 2, cex = 1.25, line = 2, outer = TRUE)
...
\begin{figure}[h!]
\begin{center}
<<plot-regions-150-2-0-5, cache = TRUE>>=
beta <- c(1.1, -1)
makeplots(n = 150, shape = 0.5, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $0.5$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
    nonparametric conformal prediction region (middle-right panel),
    LS conformal prediction region (bottom-left panel).}
...
\begin{figure}[h!]
\begin{center}
<<plot-regions-150-2-0-75, cache = TRUE>>=
makeplots(n = 150, shape = 0.75, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $0.75$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-regions-150-2-1, cache = TRUE>>=
makeplots(n = 150, shape = 1, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $1$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-regions-150-2-2, cache = TRUE>>=
makeplots(n = 150, shape = 2, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $2$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-regions-150-2-5, cache = TRUE>>=
makeplots(n = 150, shape = 5, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $2$, and bins = $5$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-regions-150-2-7, cache = TRUE>>=
makeplots(n = 150, shape = 7, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $7$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-regions-150-2-10, cache = TRUE>>=
makeplots(n = 150, shape = 10, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $10$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-regions-150-2-12, cache = TRUE>>=
makeplots(n = 150, shape = 12, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $12$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-regions-150-2-15, cache = TRUE>>=
makeplots(n = 150, shape = 15, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $15$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-regions-150-2-20, cache = TRUE>>=
makeplots(n = 150, shape = 20, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $20$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-regions-150-2-25, cache = TRUE>>=
makeplots(n = 150, shape = 25, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $25$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-misspec-150-2-5, cache = TRUE>>=
beta <- c(0.5, 1)
makeplots.misspec(n = 150, shape = 5, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $5$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-misspec-150-2-10, cache = TRUE>>=
...
\begin{figure}[h!]
\begin{center}
<<plot-misspec-150-2-20, cache = TRUE>>=
...
\begin{figure}[h!]
\begin{center}
<<plot-misspec-150-2-30, cache = TRUE>>=
makeplots.misspec(n = 150, shape = 30, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $30$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-misspec-150-2-40, cache = TRUE>>=
makeplots.misspec(n = 150, shape = 40, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $40$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-misspec-150-2-50, cache = TRUE>>=
makeplots.misspec(n = 150, shape = 50, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $50$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-misspec-150-2-60, cache = TRUE>>=
makeplots.misspec(n = 150, shape = 60, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $60$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-misspec-150-2-70, cache = TRUE>>=
makeplots.misspec(n = 150, shape = 70, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $70$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-misspec-150-2-80, cache = TRUE>>=
makeplots.misspec(n = 150, shape = 80, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $80$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-misspec-150-2-90, cache = TRUE>>=
makeplots.misspec(n = 150, shape = 90, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $90$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
\begin{figure}[h!]
\begin{center}
<<plot-misspec-150-2-100, cache = TRUE>>=
makeplots.misspec(n = 150, shape = 100, bins = 2, beta = beta)
@
\end{center}
\caption{Sim setting: $n = 150$, shape = $100$, and bins = $2$. 
  The regions are depicted as follows: 
    parametric conformal prediction region (top-left panel),
    HD prediction region (top-right panel),
    LSLW conformal prediction region (middle-left panel),
...
exploration and motivation of conformal inference. All of the R code presented 
in this document is run when this document is compiled using the linux 
terminal.
